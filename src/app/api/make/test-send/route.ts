...
export async function POST(request: Request) {
  ...
  const webhookUrl = process.env.MAKE_TEST_SEND_WEBHOOK_URL;
    ...
    return NextResponse.json(
      {
        ok: false,
        message:
          "MAKE_TEST_SEND_WEBHOOK_URL fehlt. Bitte zuerst in `.env.local` setzen.",
      },
      { status: 400 },
    );
  }
  ...
  return NextResponse.json({
    ok: response.ok,
    message: response.ok
      ? "Testversand wurde an Make.com übergeben."
      : "Make.com hat den Testversand nicht bestätigt.",
